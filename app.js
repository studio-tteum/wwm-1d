...
/* 제출 현황 표(화면 3) — 화면 1 인원 목록과 같은 행·칩 문법의 읽기 전용판.
   역할·상태를 화면 1의 라운드 칩 모양으로 맞춰 행마다 열이 정렬돼 보이게 한다.
   대기 중인 사람에게만 알림 보내기 버튼 — 제출자 행은 같은 폭 빈 칸으로 열을 지킨다. */
function renderAggPeople() {
  document.getElementById('agg-people').innerHTML = PEOPLE.map((p, i) => `
    <li class="person${p.role === 'required' ? ' required' : ''}">
      <div class="prow ro">
        ...
        ${p.submitted
          ...
          : `<button type="button" class="tool-btn mini nudge" data-i="${i}" ${p.nudged ? 'disabled' : ''}>${p.nudged ? '알림 보냄' : '알림 보내기'}</button>`}
      </div>
    </li>`).join('');
}

/* 대기 중인 사람 콕 집어 재촉 — 데모라 실제 발송 없이 보냈다는 사실만 행에 남긴다 */
document.getElementById('agg-people').addEventListener('click', e => {
  const btn = e.target.closest('.nudge');
  if (!btn || btn.disabled) return;
  PEOPLE[btn.dataset.i].nudged = true;
  renderAggPeople();
});
